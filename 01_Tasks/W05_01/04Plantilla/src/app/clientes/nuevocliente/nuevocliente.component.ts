...
export class NuevoclienteComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Obtiene idCliente para saber modo: Edicion o Nuevo
    this.idClientes = parseInt(this.ruta.snapshot.paramMap.get('idCliente') || '0', 5);

    if (this.idClientes > 0) {
      // Si idClientes> 0, modo de edición
      this.clienteServicio.uno(this.idClientes).subscribe({
        next: (uncliente) => {
          // define los valores del form con los datos del cliente obtenido
          ...
          // Update título a "Editar Cliente"
  ...
  grabar() {
    ...
    Swal.fire({
      title: 'Clientes',
      text: `Desea guardar el Cliente ${cliente.Nombres}`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        // Determinar si se inserta o actualiza el cliente by ID
